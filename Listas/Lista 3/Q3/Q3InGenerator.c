...
const char *R2Q3_file_in = "L2Q3.in";
...
{
  FILE *fileResultPtr = fopen(R2Q3_file_in, "w");
...
char *generate_string_interger_list()
{
  ...
  while (quanty > 0)
  {
    if( strcmp(add_or_remov_select(), ADD))
    {
      strcat(line, "a ");
...
{
  int addOrRemov = 0 + rand() % 10;
  return (addOrRemov > 6 ? ADD : REMOVE);
}